...
const Series = () => {
  ...
  const SeriesEntries = entriesData.entries.filter((entry) => entry.programType === 'series');
  return (
    <>
      <h2 className="w-full p-10 text-4xl	">Popular Series</h2>
      <section className="fade grid justify-items-center grid-flow-row-dense xl:grid-cols-5 md:grid-cols-3 sm:grid-cols-2 grid-cols-1  grid-rows-3 p-10 gap-10" >
        {loading ? <><Spinner /></> :
          <>
            {SeriesEntries.slice(0, 21).map((data) => (
